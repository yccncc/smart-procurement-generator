    ...
    html = `
      <div class="doc-section active" style="font-family: 'Noto Sans TC', sans-serif;">
        ...
        <p class="doc-p" style="font-size: 0.75rem; color: var(--text-muted); border-bottom: 1px solid var(--glass-border); padding-bottom: 8px; margin-bottom: 12px;">
          （依據公共工程委員會最新修訂標準投標須知格式呈現）
        </p>
        
        <p class="doc-p"><strong>一、本採購適用政府採購法(以下簡稱採購法)及其主管機關所訂定之規定。</strong></p>
        
        <p class="doc-p"><strong>二、本標案名稱：</strong><span class="h-variable" style="font-size:0.95rem; font-weight:600;">${nameDisplay}</span></p>
        
        <p class="doc-p"><strong>三、採購標的為：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">${selectedCategory === 'engineering' ? '☑' : '☐'} (1) 工程。</span><br>
          <span style="margin-left: 15px; font-weight: 500;">${selectedCategory === 'property' ? '☑' : '☐'} (2) 財物。</span><br>
          <span style="margin-left: 15px; font-weight: 500;">${selectedCategory === 'service' ? '☑' : '☐'} (3) 勞務。</span>
        </p>
        
        <p class="doc-p"><strong>四、本採購屬：</strong><span class="h-national" style="font-weight:600;">${document.getElementById('procureTierTag').innerText}</span></p>
        
        <p class="doc-p"><strong>五、本採購預算金額：</strong>新臺幣 <span class="h-variable">${budgetFormatted}</span></p>
        
        ${expansionAmount > 0 ? `
        <p class="doc-p"><strong>六、後續擴充約定：</strong>本採購保留未來向得標廠商增購之權利，後續擴充金額上限為新臺幣 <span class="h-variable">${expansionFormatted}</span>。</p>
        ` : ''}
        
        <p class="doc-p"><strong>七、招標方式為：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">${budgetAmount >= LAW_CONSTANTS.ANNOUNCEMENT_THRESHOLD ? '☑' : '☐'} (1) 公開招標。</span><br>
          <span style="margin-left: 15px; font-weight: 500;">${budgetAmount < LAW_CONSTANTS.ANNOUNCEMENT_THRESHOLD ? '☑' : '☐'} (2) 依採購法第49條規定公開取得書面報價或企劃書。本案業經核准本次公告未能取得3家以上廠商之書面報價時，改採限制性招標方式辦理。</span>
        </p>
        
        <p class="doc-p"><strong>八、共同投標：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">☑ 不允許廠商共同投標。</span>
        </p>
        
        <p class="doc-p"><strong>九、投標文件使用文字：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">☑ 中文(正體字)。</span>
        </p>
        
        <p class="doc-p"><strong>十、押標金金額：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">☑ ${bidBond > 0 ? `應繳納押標金新臺幣 <span class="h-variable">${bidBondText}</span>` : '免收押標金'}。</span>
        </p>
        
        <p class="doc-p"><strong>十一、履約保證金金額：</strong><br>
          <span style="margin-left: 15px; font-weight: 500;">☑ ${perfBond > 0 ? `應繳納履約保證金新臺幣 <span class="h-variable">${perfBondText}</span>` : '免收履約保證金'}。</span>
        </p>
        
        <p class="doc-p"><strong>十二、投標文件有效期：</strong>自投標時起至開標後 30 日止。</p>
        
        <p class="doc-p"><strong>十三、受理廠商異議之機關：</strong>同招標機關。</p>
      </div>
    `;
