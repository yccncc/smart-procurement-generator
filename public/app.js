  ...
  if (activeDocTab === 'tendering') {
    html = `
      <div class="doc-section active">
        <h1 class="doc-h1">投標須知 (工程會公告金額以上招標投標標準範本)</h1>
        <p class="doc-p">一、本案採購名稱為：<span class="h-variable">${nameDisplay}</span>。</p>
        <p class="doc-p">二、本案資金來源及預算額度：</p>
        <p class="doc-p" style="text-indent: 20px;">
          本案預算金額為新臺幣 <span class="h-variable">${budgetFormatted}</span>。
          ${expansionAmount > 0 ? `本採購案保留後續擴充額度為新臺幣 <span class="h-variable">${expansionFormatted}</span>。` : ''}
          <span class="h-national">總計採購金額 (為級距認定基準) 為新臺幣</span> <span class="h-variable">${totalFormatted}</span>，
          判定為 <span class="h-national">${document.getElementById('procureTierTag').innerText}</span> 級距。
        </p>
        ${expansionAmount > 0 ? `
        <p class="doc-p">三、後續擴充權利聲明：<br><span class="h-agency">${expansionReason}</span></p>
        ` : ''}
        <p class="doc-p">四、押標金規定：<br><span class="h-national">投標廠商應依本須知向招標機關繳納押標金：</span> <span class="h-variable">${bidBondText}</span>。</p>
        <p class="doc-p">五、履約保證金規定：<br><span class="h-national">得標廠商應於決標次日起 14 日內，繳納履約保證金：</span> <span class="h-variable">${perfBondText}</span>。</p>
      </div>
    `;
...
function exportDocuments() {
  ...
  const tenderingBody = `
    <h1>${procureName} - 招標投標須知</h1>
    
    <h2>第一章 總則與標案基本資訊</h2>
    <p>一、本案採購名稱：<span class="highlight-var">${procureName}</span></p>
    <p>二、本案採購級距：本採購案預算金額為新臺幣 <span class="highlight-var">${budgetAmount.toLocaleString('zh-TW')}元</span>。
    ${expansionAmount > 0 ? `保留後續擴充金額為新臺幣 <span class="highlight-var">${expansionAmount.toLocaleString('zh-TW')}元</span>。` : ''}
    總採購金額達新臺幣 <span class="highlight-var">${(budgetAmount + expansionAmount).toLocaleString('zh-TW')}元</span>，
    屬 <span class="highlight-var">${document.getElementById('procureTierTag').innerText}</span>。</p>
    
    <h2>第二章 廠商投保與保證金細則</h2>
    <p class="highlight-nat">三、押標金繳納：廠商應於截標期限前，向機關繳納押標金為新臺幣：<strong>${bidBond > 0 ? bidBond.toLocaleString('zh-TW') + '元' : '免收'}</strong>。</p>
    <p class="highlight-nat">四、履約保證金繳納：得標廠商應於決標次日起 14 日內，繳納履約保證金新臺幣：<strong>${perfBond > 0 ? perfBond.toLocaleString('zh-TW') + '元' : '免收'}</strong>。</p>
    
    <h2>第三章 其他規定</h2>
    ${expansionAmount > 0 ? `<p class="highlight-age">五、後續擴充說明：${expansionReason}</p>` : ''}
    <p>六、本投標須知未規範事項，悉依中華民國政府採購法及公共工程委員會最新範本辦理。</p>
  `;
